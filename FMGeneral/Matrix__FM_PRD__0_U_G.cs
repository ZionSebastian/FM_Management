...
namespace FMGeneral
{
    ...
    public class Matrix__FM_PRD__0_U_G : B1Item
    {

        public Matrix__FM_PRD__0_U_G()
        {
            FormType = "FM_PRD";
            ItemUID = "0_U_G";
        }
        ...
        public virtual void OnAfterValidate(ItemEvent pVal)
        {
            ...
            try
            {
                if (pVal.InnerEvent == false)
                {
                    ...
                    var with1 = form.DataSources.DBDataSources.Item("@FM_PRD1");
                            ...
                            case "C_0_1":
                                {
                                    ...
                                    string LastItem = with1.GetValue("U_FPMonth", lastrow -1).ToString().Trim();
                                    if (LastItem != "")
                                    {
                            ...
                            case "C_0_2":
                                {
                                    ...
                                    {
                                        TtlFPTon += Convert.ToDouble(with1.GetValue("U_Tons", i).ToString().Trim());
                                        
                                        with.SetValue("U_TtlFPTon", 0, TtlFPTon.ToString().Trim());
                            ...
                            case "C_0_3":
                                {
                                    ...
                                    for (int i = 0; i < oMat.RowCount; i++)
                                    {
                                        if (with1.GetValue("U_Heat", i).ToString().Trim() != "")
                                        {
                                            TtlFPHeat += Convert.ToDouble(with1.GetValue("U_Heat", i).ToString().Trim());
                                        }
                                        with.SetValue("U_TtlFPHeat", 0, TtlFPHeat.ToString().Trim());
